						...
						<div class="comment-body" id="comment-body-{{ $post->id }}">
							...
							@foreach ($post->comments as $comment)
								...
								<div class="comment mx-3 pt-2" id="{{ $comment->id }}">
									<span><a href="#" class="h6">{{ $comment->user->name }}</a> said on: {{ $comment->created_at->toFormattedDateString() }}, {{ $comment->created_at->setTimezone(Auth::user()->timezone)->format('g:i A') }}</span>
									...
									<p id="comment_{{ $comment->id }}" class="pt-2 comment-paragraphs">{{ $comment->comment }}</p>
								</div>
							@endforeach
						</div>
					</div>
